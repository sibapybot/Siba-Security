...
async def avatar_check(member):
    ...
    if avatar_url != None:#デフォルトのアバターはパスする
        ...
        if s:
            l=list(csv_read(guild_id=guild_id))
            img_avatar_now = cv2.imread(path) 
            for i in range(len(l)):
                
                img__avatar_old = cv2.imread(fr"C:\Users\waon-pc\Desktop\sibainu_discord\siba_bot\data\avatar_img\{l[i]}.png") 
    ...
    l.append(user_id)
    csv_write(data=l,guild_id=guild_id)
    return 3#一致なし
    ...
    for z in range(len(message_list)):
        for i in range(len(message_list)):
            if id_message_list[z] != id_message_list[i]:
                ...
                message_ave.append(difflib.SequenceMatcher(None, message_list[z], message_list[i]).ratio())
