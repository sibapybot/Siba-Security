...
class User_Command(commands.Cog):
    ...
    async def user(self,interaction: discord.Interaction,user_id:str):
       # try:
            ...
            embed = discord.Embed(title="ユーザー情報",description=f"{name}さんの情報",color=color)
            embed.add_field(name="アカウント作成日",value=f"<t:{int(created_at.timestamp())}>")
            embed.add_field(name="ユーザーID",value=f"`{user_id}`")
            embed.set_thumbnail(url=avatar)
       # except:
       #     embed = discord.Embed(title="エラー",description="ユーザー情報を取得できませんでした。",color=0xfc2a00)
