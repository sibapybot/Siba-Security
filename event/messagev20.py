...
class MessageV20(commands.Cog):
    ...
    async def member_timeout(self,guild:discord.guild):
        ...
        while nonecount <= 30:
            print(nonecount)
            TOlist = [i[1] for i in self.tmp_guild[guild.id]["tmp_m"] if i[2] == 0 and i[1].id not in self.tmp_guild[guild.id]["TO_member"]]
            if not TOlist:
                nonecount += 1
            else:
                nonecount = 0
    ...
    async def purge_msg(self,guild_id:int):
        ...
        embed = discord.Embed(title="メッセージの削除が完了しました。",description="荒らしと判断したメッセージは削除が完了しました。",color=0x00e626)
        await self.tmp_guild[guild_id]["guild"].system_channel.send(embed = embed)
    ...
    async def reto_ch(self,guild_id):
        ...
        embed = discord.Embed(title="低速モード解除",description="低速モードが自動解除されました。\nもともと低速モードを設定していた場合は元の値に戻りました。",color=0x00e626)
    ...
    async def embed_create(self,
        ...
        if guild.id not in self.tmp_guild:
            embed = discord.Embed(title="荒らしを検知しました。",description="発言はお控えください。",color=0xe62600)
    ...
    async def on_message(self,message):
        ...
        if guild.id in self.tmp_message:
            time_a = datetime.timedelta(hours=now.hour,minutes=now.minute,seconds=now.second)
            #30秒以上経ったメッセージの削除
            to_message:list = []
            for i in range(len(self.tmp_message[guild.id])):
                dt = self.tmp_message[guild.id][i][1]
                total_seconds_a=time_a.total_seconds()
                time_c = datetime.timedelta(hours=dt[3],minutes=dt[4],seconds=dt[5])
            ...
            time_c = datetime.timedelta(hours=dt[3],minutes=dt[4],seconds=dt[5])
            text_list = [i[0] for i in self.tmp_message[guild.id] if time_a - time_c <= datetime.timedelta(seconds=30)]
        ...
        #トークンを貼ると脅威が+1000
        if re.match(r"[0-9a-zA-Z_\-]{24}.[0-9a-zA-Z_\-]{6}.[0-9a-zA-Z_\-]{27} OR [0-9a-zA-Z_\-]{24}.[0-9a-zA-Z_\-]{6}.[0-9a-zA-Z_\-]{38}",message.content):
            point += 1000

        if guild.id not in self.tmp_message:
            self.tmp_message[guild.id]=[]
        #メッセージの追加
        self.tmp_message[guild.id].append([message.content,[now.year,now.month,now.day,now.hour,now.minute,now.second]])
