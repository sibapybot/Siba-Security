...
class User_Join(commands.Cog):
    ...
    async def on_message(self,message:discord.message):
        ...
        tmp_n = [line.split()[0] for line in m.parse(member.name).splitlines() if "名詞" and not "名詞-数" in line.split()[-1]]
        names = [mbr.name for mbr in guild.members]
        for s in range(len(names)):
            tmp = [line.split()[0] for line in m.parse(names[s]).splitlines() if "名詞" and not "名詞-数" in line.split()[-1]]
            del tmp[-1] 
            names[s] = tmp
        
        for z in names:
            if z == tmp_n:
                embed.add_field(name="名前がほぼ一致いるユーザーがいませんか？",value="対象ユーザーはタイムアウトされます。")
