...
class MessageV20(commands.Cog):
    ...
    async def member_timeout(self, guild: discord.guild):
        ...
        async def m_to(member):
            try:
                ...
                logger.error("Could not timeout.")
    ...
    async def purge_msg(self, guild_id: int):
        logger.info(f"Begin delete message in guildid:{guild_id}")
        ...
        logger.info(f"Delete: id:{msg_id} channel:{channel}")
        embed = discord.Embed(
    ...
    async def reto_ch(self, guild_id):
        ...
        for ch_data in data["slowchannel"]:
            ...
            logger.info(f"Disable slowmode:{ch_data.id} in {guild_id}")
        embed = discord.Embed(
            title="低速モード解除",
    ...
    async def embed_create(self,
        ...
        logger.info(guild.id in self.tmp_guild)
    ...
    async def on_message(self, message):
        ...
        if target in message.content:
            ...
            point += 20
        # 招待リンクを貼ると脅威度が+20
        target = "https://discord.com/invite/"
        if target in message.content:
            idx = message.content.find(target)
            r = message.content[idx + len(target):]
            re.match("[0-9a-zA-Z]{7,10}", r)
        ...
        # あらしと検知されたらif分岐
        if point >= 80:
            logger.info(f"Timeout:{member.id}")
            await self.embed_create(member=member, message=message)

            logger.info(
                f"Alltime:{time.time() - start_time}\n{time.time()-time2}")
